...
const Products = (props) => {
  ...
  const addToCart = (e) => {
    ...
    console.log(`cart started as ${JSON.stringify(cart)}`)
    let name = e.target.name;
    console.log(`name is ${JSON.stringify(name)}`);
    let item = items.filter((item) => item.name == name);
    console.log(`Item in stock is ${item[0].instock}`);
    if (item[0].instock !== 0){
  ...
  /*  
    ...
    console.log(`cart now is ${JSON.stringify(cart)}`);
*/
  ...
  const deleteCartItem = (index) => {
    ...
    setCart(newCart);
/*
    let itemToDelete = cart.filter((item, i) => index === i);
    console.log(`itemTodelete ${JSON.stringify(itemToDelete)}`);
    let updatedStock = items.map((item) =>{
      if(item.name == itemToDelete[0].name){
        item.instock = item.instock + 1;
      }
        return item;
    });
    console.log(`items are ${JSON.stringify(items)}`)
    console.log(`updatedStock is ${JSON.stringify(updatedStock)}`)
